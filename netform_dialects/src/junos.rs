//! Juniper Junos: parser and registry data.
//!
//! Junos is one of the two vendors whose grammar needs real code rather than a
//! rule table — brace blocks, `/* … */` comment regions spanning lines, and
...
///
/// ```rust
/// use netform_dialects::junos::literal_region;
///
/// let region = literal_region("    certificate \"-----BEGIN CERTIFICATE-----").unwrap();
/// assert!(region.terminates("-----END CERTIFICATE-----\";"));
/// assert!(literal_region("    description \"uplink\";").is_none());
/// ```
...
///
/// ```rust
/// use netform_dialects::junos::comment_region;
///
/// let region = comment_region("/* rack notes").unwrap();
/// assert!(region.terminates("   row B */"));
/// assert!(comment_region("/* rack notes */").is_none());
/// ```
pub fn comment_region(raw: &str) -> Option<LiteralTerminator> {
...
mod tests {
    ...
    fn literal_region_opens_on_an_unclosed_quoted_value() {
        assert_eq!(
            literal_region("                certificate \"-----BEGIN CERTIFICATE-----"),
            Some(LiteralTerminator::UnescapedQuote),
        );
        assert_eq!(
            literal_region("        ssh-rsa \"ssh-rsa AAAAB3Nz"),
            Some(LiteralTerminator::UnescapedQuote),
        );
        assert_eq!(
            literal_region("set system login announcement \"Authorized use only"),
            Some(LiteralTerminator::UnescapedQuote),
        );
    }
    ...
    fn literal_region_declines_self_contained_values() {
        assert_eq!(literal_region("    description \"uplink\";"), None);
        assert_eq!(literal_region("set snmp location \"rack 4\""), None);
        assert_eq!(literal_region("interfaces {"), None);
    ...
    fn literal_region_declines_unquoted_junos_statements() {
        assert_eq!(literal_region("    apply-groups GRP-DEFAULTS;"), None);
        assert_eq!(literal_region("    apply-groups-except RE1;"), None);
        assert_eq!(
            literal_region("annotate interfaces \"managed by netops\""),
            None,
        );
        assert_eq!(literal_region("    inactive: disable;"), None);
        assert_eq!(
            literal_region("set protocols bgp group PEERS neighbor 10.0.0.1"),
            None,
        );
    }
    ...
    fn literal_region_opens_on_an_escaped_quote_in_the_opener() {
        assert_eq!(
            literal_region(r#"    message "say \"hello\" first"#),
            Some(LiteralTerminator::UnescapedQuote),
        );
        assert_eq!(
            literal_region(r#"    message "say \"hello\" first";"#),
    ...
    fn literal_region_declines_a_comment_holding_an_odd_quote() {
        assert_eq!(literal_region("## Last changed by \"admin"), None);
        assert_eq!(literal_region("# unbalanced \" here"), None);
        assert_eq!(literal_region("/* he said \" and left"), None);
        assert_eq!(literal_region(" * still a \" comment"), None);
        assert_eq!(literal_region("*/ trailing \""), None);
    }
    ...
    fn comment_region_opens_on_an_unclosed_block_comment() {
        assert_eq!(comment_region("/* site notes"), comment_terminator());
        assert_eq!(comment_region("    /* uplink notes"), comment_terminator(),);
        assert_eq!(comment_region("/*"), comment_terminator());
    }

    #[test]
    fn comment_region_declines_a_self_contained_block_comment() {
        assert_eq!(comment_region("/* one line */"), None);
        assert_eq!(comment_region("/**/"), None);
        assert_eq!(comment_region("/* a */ /* b"), None);
    }

    #[test]
    fn comment_region_declines_anything_not_opening_with_a_block_comment() {
        assert_eq!(comment_region("## Last changed by admin"), None);
        assert_eq!(comment_region(" * continuation"), None);
        assert_eq!(comment_region("*/"), None);
        assert_eq!(comment_region("interfaces {"), None);
        assert_eq!(comment_region("    host-name router-1; /* note"), None);
        assert_eq!(comment_region(""), None);
    }

    #[test]
    fn a_comment_region_terminator_matches_a_closer_anywhere_on_its_line() {
        let region = comment_region("/* site notes").expect("an open comment");
    ...
    fn parse_junos_sets_named_dialect_hint() {
        let doc = parse("set system host-name router-1\n");
        assert_eq!(
