...
- `--dialect auto` now recognizes Cisco IOS XR. it scores on constructs no other supported vendor has — Routing Policy Language and its `*-set` families, the BGP group templates, `ipv4 address`/`ipv4 access-list`, the XR-only interface families (`Bundle-Ether`, `BVI`, `MgmtEth`, `PW-Ether`, `tunnel-ip`) and the XR-only Ethernet spellings (`TenGigE0/0/0/0`, `FortyGigE0/0/0/0`). an interface named the way IOS XE names one still reads as IOS XE however deep its slot notation, so a Catalyst breakout subport — `HundredGigE1/0/20/1`, `TwentyFiveGigE1/0/20/1` — is untouched, as is the three-part naming IOS XE and NX-OS use
