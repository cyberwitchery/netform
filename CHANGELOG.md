...
- `--dialect auto` now recognizes Cisco IOS XR. it scores on constructs no other supported vendor has — Routing Policy Language and its `*-set` families, the BGP group templates, `ipv4 address`/`ipv4 access-list`, the XR-only interface families (`Bundle-Ether`, `BVI`, `MgmtEth`, `PW-Ether`, `tunnel-ip`) and any interface name in XR's four-part rack/slot/instance/port notation whose type no other supported dialect spells (`TenGigE0/0/0/0`, `FortyGigE0/0/0/0`). a name another supported vendor spells does not score XR however deep its slot notation: a Catalyst breakout subport — `HundredGigE1/0/20/1`, `TwentyFiveGigE1/0/20/1` — still reads as IOS XE, `Ethernet1/2/3/4` still reads as NX-OS, and a four-part `Serial`, `Vlan`, `Port-channel`, `Loopback`, `Tunnel`, `Management` or `Vxlan` scores nothing at all, as does the three-part naming IOS XE and NX-OS use
