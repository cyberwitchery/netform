...
- `--format markdown` now escapes the text it quotes — configuration lines, file labels and key labels — so a report renders what the device actually holds. none of it was escaped before: `_core_` rendered italic with the underscores gone, `` `code` `` opened a code span, `<b>halt</b>` passed through as raw HTML, `&nbsp;` became a space, and an interface named `<ge-0>` or an edit `at key <unknown>` lost the bracketed word to a tag. a file path containing a backtick also broke out of its label's code span. `--format unified` output is unchanged
- a whitespace-only change inside a replaced line now renders as a code span in markdown reports instead of `**` markers: `**  **` is not emphasis under CommonMark, so a change from `set mtu  1500` to `set mtu 1500` used to render as literal asterisks with the differing spaces collapsed, leaving the two sides looking identical
