...
- **breaking:** deprecated `netform_ir::detect::auto_parse` — it detects the dialect but always uses the generic parser, discarding dialect-specific trivia classification, tokenization, and key hints; callers should use `detect_dialect()` and dispatch to the appropriate dialect parser directly (the CLI crate's `parse_config` with `CliDialect::Auto` demonstrates the correct pattern)
