...
- FortiOS multi-line quoted values (`set private-key "…"`, replacement-message `set buffer "…"`, banner text) are read as free text up to the closing quote. every physical line used to parse as configuration, so a flush-left certificate body dedented out of its `config` and `edit` blocks and the closing-quote line became a block header — `next` then never closed its `edit`. nothing signalled it: the round trip stayed byte-perfect while every later section sat at the wrong path in plan and diff output
...
- IOS-family banners (`banner motd ^C` … `^C`, the glued `banner motd #Warning restricted` … `#`, and the delimiter-less Arista form) are read as free text on Arista EOS, Cisco NX-OS and Cisco IOS XE. a banner line beginning with `!` or `#` was dropped by `--ignore-comments`, so a real edit to the banner produced no output and exit 0, and one reading `interface GigabitEthernet0/0/0` claimed the real interface's identity. banner text is now compared verbatim
...
- fixed Junos `set`-style statements sharing one identity per section (`set system …`, `set routing-options …`, `set security …` and others), which under `--order-policy keyed-stable` reported changes to untouched settings while hiding real ones. identity now derives from the statement path with the trailing value excluded, so a value edit reads as a replace; statements whose trailing token is a list member (name servers, ssh ciphers, `host-inbound-traffic` services) key on their full text so distinct members stay distinct
