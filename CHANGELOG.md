...
- fixed Junos `set`-style statements sharing one identity per section (`set system …`, `set routing-options …`, `set security …`, `set snmp …`, `set chassis …`, `set class-of-service …`, `set forwarding-options …`), which under `--order-policy keyed-stable` reported changes to untouched settings while hiding real ones. identity now derives from the statement path with the trailing value excluded, so a value edit reads as a replace: a static route keys on its prefix and attribute name (`qualified-next-hop` attributes also on the next hop's address; `discard`/`reject`/`receive` on the route alone), and statements whose trailing token is a list member (name servers, ssh ciphers, ssh keys, `host-inbound-traffic` services) key on their full text so distinct members stay distinct
