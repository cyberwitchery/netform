...
- `--dialect auto` now recognizes Cisco IOS XR. it scores on constructs no other supported vendor has — Routing Policy Language and its `*-set` families, the BGP group templates, `ipv4 address`/`ipv4 access-list`, the XR-only interface families (`Bundle-Ether`, `BVI`, `MgmtEth`, `PW-Ether`, `tunnel-ip`) and any interface name in XR's four-part rack/slot/instance/port notation whose type no other supported dialect spells (`TenGigE0/0/0/0`, `FortyGigE0/0/0/0`), in any case. a name another supported vendor spells does not score XR however deep its slot notation: a Catalyst breakout subport — `HundredGigE1/0/20/1`, `HundredGigabitEthernet1/0/2/1`, `FourHundredGigE1/0/31/1`, `TwoHundredGigE1/0/5/1`, `TwentyFiveGigE1/0/20/1` — still reads as IOS XE, `Ethernet1/2/3/4` still reads as NX-OS, and a four-part `Serial`, `Vlan`, `Port-channel`, `Loopback`, `Tunnel`, `Management` or `Vxlan` scores nothing at all, as does the three-part naming IOS XE and NX-OS use
- `netform_dialect_iosxe`, `netform_dialect_nxos` and `netform_dialect_eos` now export their interface-type tables (`IOSXE_INTERFACE_TYPES`, `NXOS_INTERFACE_TYPES`, `EOS_INTERFACE_TYPES`), and a new `netform_cli` suite asserts that none of those types is read as IOS XR on slot shape alone. `--dialect auto`'s IOS XR signal shadows a hand-maintained copy of those tables, which `netform_ir` cannot read — the dialect crates depend on it, not the other way round — so the copy could drift from the tables it mirrors with nothing to notice. adding an interface type to a dialect crate without widening that guard now fails a test instead of silently changing which vendor `--dialect auto` reports for that type's four-part names
